...
# non-quote constants
	...
	#s/^>>>//
:loop
	s/\(|3 \)\([0-9][0-9A-Fa-f]*\)H/\10x\2/
	s/\([,+-]\)\([0-9][0-9A-Fa-f]*\)H/\10x\2/
	/^[0-9][0-9A-Fa-f]*H/b loop
	b
}
' |sed '
#
...
1i\
|1 |2 .area|3 TEST (ABS)
/|2 DB|3 "/s/\(|2 \)DB\(|3\)/\1.ascii\2/
s/|2 ORG|/|2 .org|/
s/|2 EQU|/|2 .equ|/
s/|2 DW|/|2 .dw|/
s/|2 DB|/|2 .db|/
' |sed '
# re-format column spacing
	/^|1 |2 |3 |;/b end
	s/^|1 \([^|]*\)|2 \([^|]*\)|3 \("[^|]*\)|\(;.*\)$/|1 \1        |2 \2                   |3 \3|\4/
	t format
	s/^|1 \([^|]*\)|2 \([^|]*\)|3 \([^|]*\)|\(;.*\)$/|1 \1        |2 \2                   |3 \3                              |\4/
	t form3
	s/^|1 \([^|]*\)|2 \([^|]*\)|3 \("[^|]*\)$/|1 \1                |2 \2                   |3 \3/
...
:format
	s/^\(|1 ........\)[^|]*\(|2 ........\)[^|]*/\1\2/
:end
	# final formating, remove |1,|2,|3,|; markers
	s/^|1 \([^|]*\)|2 \([^|]*\)|3 \([^|]*\)|;/\1\2\3;/
	s/^|1 \([^|]*\)|2 \([^|]*\)|3 \([^|]*\)/\1\2\3/
	s/^|1 \([^|]*\)|2 \([^|]*\)/\1\2/
'
